...
public class LocalFileStorageService implements FileStorageService {
  ...
  public StoredFile store(MultipartFile file) {
    ...
    String storagePath = localRoot.relativize(targetFile).toString().replace('\\', '/');
  ...
  private void validateFile(MultipartFile file) {
    ...
    if (file.getSize() > storageProperties.getMaxFileSizeBytes()) {
      throw new BusinessException(
          FILE_TOO_LARGE_CODE,
          "Uploaded file size exceeds limit: "
    ...
    if (!allowedExtensions().contains(fileType)) {
      throw new BusinessException(
          FILE_TYPE_NOT_ALLOWED_CODE, "File extension is not allowed: " + fileType);
    }
  }
  ...
  private Set<String> allowedExtensions() {
    return storageProperties.getAllowedExtensions().stream()
